...
class PlentymarketsImportEntityItem
{
    ...
    public function saveItemTextsTranslation($itemTexts)
    {
        ...
        foreach ($itemTexts as $itemText) {
            ...
            if (isset($itemText['languageShopId'])) {
                ...
                $data = [$swItemID, $itemText['languageShopId'], $swItemText['txtArtikel']];
            ...
            if (!is_null($shopId)) {
                ...
                try {
                    ...
                    if (!is_null($shopware_propertyValueID)) {
                        ...
                        $property_data = ['optionValue' => $ItemProperty->PropertyValue];
    ...
    public function importCategories()
    {
        ...
        Shopware()->Db()->query('DELETE FROM s_articles_categories_seo WHERE shop_id = ? AND article_id = ?', [$this->Shop->getId(), $SHOPWARE_itemID]);

        if (!empty($this->seoCategories)) {
            Shopware()->Db()->query('INSERT INTO s_articles_categories_seo (shop_id,article_id,category_id) VALUES(?,?,?)', [$this->Shop->getId(), $SHOPWARE_itemID, $this->seoCategories[0]['categoryId']]);
        }
    ...
    public function import()
    {
        ...
        try {
            ...
            if (count($this->variants)) {
                ...
                if ($numberOfVariantsUpdated == 0) {
                    $Article = $ArticleResource->update($SHOPWARE_itemID, [
                        'configuratorSet' => [
                            'groups' => [],
                        ],
                        'variations' => [],
                ...
                foreach ($article['details'] as $detail) {
                    ...
                    elseif (isset($number2sku[$detail['number']])) {
                        // Add the mapping
                ...
                } elseif ($numberOfVariantsUpdated > 1) {
                ...
                } elseif ($numberOfVariantsCreated > 1) {
                ...
                } elseif ($numberOfVariantsDeleted > 1) {
        ...
        Shopware()->Events()->notify('PlentyConnector_ImportEntityItem_AfterImpoert', [
            'subject' => $this,
        ...
        foreach ($this->ItemBase->AttributeValueSets->item as $AttributeValueSet) {
            ...
            try {
                ...
                if (PlentymarketsConfig::getInstance()->getItemNumberImportActionID(IMPORT_ITEM_NUMBER) == IMPORT_ITEM_NUMBER) {
                    ...
                    $number = Shopware()->Db()->fetchOne('SELECT ordernumber FROM s_articles_details WHERE id = ?', [$details['id']]);
                }
            } catch (PlentymarketsMappingExceptionNotExistant $e) {
                // Numbers should be synced
                if (PlentymarketsConfig::getInstance()->getItemNumberImportActionID(IMPORT_ITEM_NUMBER) == IMPORT_ITEM_NUMBER) {
    ...
    protected function setProperties()
    {
        ...
        foreach ($this->ItemBase->ItemProperties->item as $ItemProperty) {
            ...
            } elseif (!$ItemProperty->ShowOnItemPageInWebshop) {
        ...
        try {
            ...
            // Create the group
            $GroupAdded = Shopware\Components\Api\Manager::getResource('PropertyGroup')->create([
